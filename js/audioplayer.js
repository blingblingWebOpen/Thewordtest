		...
		Plugin.prototype = {
			init: function() {
				...
				if(self.settings.src !== ''){
					...
					var avinfo;
					var name = self.settings.src;
					var args = name.substr(name.indexOf("?") + 1).split("&");
					for ( var i = 0; i < args.length; i++) {
					    var j = args[i].indexOf("=");
					    if (j > -1 && args[i].substr(0, j) == 'avinfo') {
					        avinfo = args[i].substr(j + 1);
					    }
					}
					avinfoall = avinfo;
					alert(avinfo);
			...
			animateProgressBarPosition: function() {
				...
				if(curs<10){
					...
					curs=String(0)+String(curs)-Math.floor(curs/60)*60;
				}
			...
			updateTotalTime: function() {
				var self = this,
					...
					alert("durationtime  "+durationtime);					
